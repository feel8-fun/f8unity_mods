...
def _infer_exporter_from_existing_profiles(game_root: Path) -> str:
    ...
    for fallback_key, path in candidates:
        ...
        try:
            ...
            log(f"warning: cannot inspect existing profile {path}: {exc}")
            continue
    ...
    if detection.bepinex_partial:
        if not bepinex_variant_matches_backend(detection.backend, detection.bepinex_variant):
            if force_reinstall:
                ...
                blocking.append("Incomplete BepInEx files mismatch the game backend; use --force-reinstall.")
        else:
            actions.append("repair_incomplete_bepinex")
    elif detection.has_bepinex:
        if not bepinex_variant_matches_backend(detection.backend, detection.bepinex_variant):
            if force_reinstall:
                actions.append("backup_and_reinstall_bepinex")
            else:
...
def _install_exporter_plugin(
    ...
    *,
    ...
    except (OSError, ValueError, SetupError, zipfile.BadZipFile) as e:
        log(f"warning: remote exporter unavailable; using local build artifact: {e}")
...
def run_install(
    ...
    *,
    ...
    has_mismatched_bepinex = (detection.has_bepinex or detection.bepinex_partial) and not bepinex_variant_matches_backend(
        detection.backend, detection.bepinex_variant
    )
    if has_mismatched_bepinex:
    ...
    elif detection.bepinex_partial:
        detection = _install_bepinex(detection, config, offline=offline)
        summary["actions"].append({"install_bepinex": "repaired_incomplete"})
    ...
    summary["bepinex_status"] = _bepinex_status_payload(detection)
    summary["bepinex_variant"] = detection.bepinex_variant
    summary["bepinex_version"] = detection.bepinex_version
...
def _install_bepinex(detection: DetectionResult, config: SetupConfig, offline: bool) -> DetectionResult:
    ...
    if not refreshed.has_bepinex:
        missing = ", ".join(refreshed.bepinex_missing_components) or "unknown loader components"
        raise SetupError(
            EXIT_INSTALL_FAILED,
            f"BepInEx install finished but the loader is incomplete; missing: {missing}",
        )
    return refreshed


def _bepinex_status_payload(detection: DetectionResult) -> dict[str, Any]:
    return {
        "installed": detection.has_bepinex,
        "partial": detection.bepinex_partial,
        "variant": detection.bepinex_variant,
        "version": detection.bepinex_version,
        "major": detection.bepinex_major,
        "corePresent": detection.bepinex_core_present,
        "bootstrapPresent": detection.bepinex_bootstrap_present,
        "coreFiles": list(detection.bepinex_core_files),
        "bootstrapFiles": list(detection.bepinex_bootstrap_files),
        "missingComponents": list(detection.bepinex_missing_components),
    }
